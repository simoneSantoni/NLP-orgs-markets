...
"""
-------------------------------------------------------------------------------
    _5.py    |    example of Minimum Edit Distance application
-------------------------------------------------------------------------------

Author   : Simone Santoni, simone.santoni.1@city.ac.uk

Synopsis : Appreciating the approximate or phonetic distance between two string
           ...
           One possible application is identifying successful patch/code
           submissions on the part of developers or data scientists working in a
           team setting (e.g., GitHub teams). For example, the members of the
           Linux Kernel community exchange their code using the mailing list of
           the project, while very few members have the power to accept (i.e.,
           to commit) a patch/code submissions. This raises concerns on who
...
def get_code(_s):
    ...
    """
    _split = _s.split("\n")
    _code = [line.strip(">").strip() for line in _split if line.startswith("> ")]
    return " ".join(_code)
...
# print results
print(
    """Email - commit a distance: {}
Email - commit b distance: {}""".format(
        s1_s2, s1_s3
    )
)
